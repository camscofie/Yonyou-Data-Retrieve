...
Sub 明细帐()
    ...
    For Each c In arr
        ...
        With Worksheets(str_sht)    '统计借方　贷方金额
            ...
            Worksheets("明细帐").Cells(R, 8).Value = WorksheetFunction.Sum(.Range(.Cells(2, 9), .Cells(lon_row, 9)))
            Worksheets("明细帐").Cells(R, 9).Value = WorksheetFunction.Sum(.Range(.Cells(2, 10), .Cells(lon_row, 10)))

        End With
        Call 帐的期初期末金额(str_sht, "明细帐", R)
        Call 插入期初数(str_sht)
        With Worksheets("凭证")    '帐本链接凭证
            For W = 2 To lon_row
                For x = 2 To lon_pz
...
Sub 末级科目()
    ...
    Dim lon_row As Long, lon_zzkm As Long
    Dim Dbl_yu
    lon_zzkm = Worksheets("凭证").Cells(1, 16).Value
    ...
    With Worksheets("凭证")
        ...
        lon_pz = .UsedRange.Rows.Count
    End With
    arr = oCnn.Execute("select distinct 末级科目 from " & sTab).GetRows
    Worksheets.Add(after:=Worksheets("凭证")).Name = "末级科目"
    Worksheets("末级科目").Cells(1, 1).Rows.Value = "序号"
    Worksheets("末级科目").Cells(1, 2).Rows.Value = "总帐编码"
    Worksheets("末级科目").Cells(1, 3).Rows.Value = "总帐科目"
    Worksheets("末级科目").Cells(1, 4).Rows.Value = "期初借方额"
    Worksheets("末级科目").Cells(1, 5).Rows.Value = "期初贷方额"
    Worksheets("末级科目").Cells(1, 6).Rows.Value = "借方发生额"
    Worksheets("末级科目").Cells(1, 7).Rows.Value = "贷方发生额"
    Worksheets("末级科目").Cells(1, 8).Rows.Value = "期未借方额"
    Worksheets("末级科目").Cells(1, 9).Rows.Value = "期未借方额"
    R = 2
    For Each c In arr

        Worksheets("末级科目").Cells(R, 1).Rows.Value = R - 1
        Worksheets("末级科目").Cells(R, 3).Rows.Value = c
        With Worksheets("凭证")
            .Range(.Cells(1, 1), .Cells(.UsedRange.Rows.Count, 15)).AutoFilter Field:=8, Criteria1:=c
            .Range(.Cells(1, 1), .Cells(.UsedRange.Rows.Count, 15)).Copy
        End With
        Worksheets.Add(after:=Sheets(Sheets.Count)).Name = c
        Worksheets(c).Range("A1").Select
        ...
        With Worksheets(c)    '统计借方　贷方金额
            ...
            Worksheets("末级科目").Cells(R, 6).Value = .Cells(lon_row + 1, "i")
            Worksheets("末级科目").Cells(R, 7).Value = .Cells(lon_row + 1, "j").Value
            Worksheets("末级科目").Cells(R, 3).Hyperlinks.Add Anchor:=Worksheets("末级科目").Cells(R, 3), Address:="", SubAddress:=.Name & Chr("33") & "a2"
            Worksheets("末级科目").Cells(R, 2).Value = Mid(.Cells(2, "f"), 1, lon_zzkm)
            Dbl_yu = (Worksheets("末级科目").Cells(R, 4).Value + Worksheets("末级科目").Cells(R, 6).Value) - (Worksheets("末级科目").Cells(R, 5).Value + Worksheets("末级科目").Cells(R, 7).Value)
            If Dbl_yu > 0 Then
                Worksheets("末级科目").Cells(R, 8).Value = Dbl_yu
            Else
                Worksheets("末级科目").Cells(R, 9).Value = Abs(Dbl_yu)
            End If
...
ok:
    ...
    Worksheets("凭证").Select
    Selection.AutoFilter
    Worksheets("末级科目").Select
    Worksheets("末级科目").Range(Cells(2, 1), Cells(R, 9)).Select
    Call 排序
    For i = 2 To R - 1
        Worksheets("末级科目").Cells(i, 1).Rows.Value = i - 1
    Next

    Worksheets("末级科目").Range("f2").Select
End Sub
